...
function SWEP:PrimaryAttack()
    ...
    self:EmitSound("weapons/357/357_fire2.wav", 140, 100, 1, CHAN_WEAPON)
    local extrasound = ents.Create("base_gmodentity")
    extrasound:Spawn()
    extrasound:SetPos(self:GetOwner():GetShootPos())
    extrasound:EmitSound("weapons/smg1/smg1_fire1.wav", 140, 85, 1, CHAN_WEAPON)
    extrasound:Remove()
	...
	if ads == 0 then
	    ...
	    self:GetOwner():SetEyeAngles(self:GetOwner():EyeAngles() + Angle(math.Rand(-2, -1.25), math.Rand(-0.65, 0.65), 0))
	end
    end
end
