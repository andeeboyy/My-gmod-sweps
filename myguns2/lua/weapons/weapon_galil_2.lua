...
function SWEP:PrimaryAttack()
	...
	if ads == 0 then
            self:GetOwner():SetEyeAngles(self:GetOwner():EyeAngles() + Angle(math.Rand(-4.25, -2.25), math.Rand(-2.5, 2.5), 0))
	else
	    self:GetOwner():SetEyeAngles(self:GetOwner():EyeAngles() + Angle(math.Rand(-2.75, -2.25), math.Rand(-1, 1), 0))
	end
    end
end
