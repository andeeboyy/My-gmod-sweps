...
function SWEP:GetViewModelPosition(pos, ang)
    if self:GetOwner():KeyDown(IN_ATTACK2) and ads == 1 and !self:GetOwner():IsSprinting() and reloading == 0 then
	spronetime = 1
	moveside = math.Approach(moveside, -5.755, 0.05)
	movevertical = math.Approach(movevertical, 3, 0.225)
	...
	if self:GetOwner():IsSprinting() then
	    ...
	    movevertical = math.Approach(movevertical, 2, 0.05)

	    turn = math.Approach(turn, -5, 0.1)
	...
	spronetime = 1
	moveside = math.Approach(moveside, -4, 0.1)
	movevertical = math.Approach(movevertical, 2, 0.05)
