...
function SWEP:Think()
    ...
    if self:GetOwner():KeyPressed(IN_ATTACK2) and canads == 1 and !self:GetOwner():IsSprinting() and timerRunning == 0 then
	ads = 1
...
function SWEP:GetViewModelPosition(pos, ang)
    if self:GetOwner():KeyDown(IN_ATTACK2) and ads == 1 and !self:GetOwner():IsSprinting() then
	spronetime = 1
	moveside = math.Approach(moveside, -6.62, 0.05)
	movevertical = math.Approach(movevertical, 2.25, 0.225)

	turn = math.Approach(turn, 2.25, 0.05)
	...
	if self:GetOwner():IsSprinting() then
	    ...
	    moveside = math.Approach(moveside, -1, 0.05)
	    movevertical = math.Approach(movevertical, 6, 0.05)

	    turn = math.Approach(turn, -10, 0.1)
	...
	spronetime = 1
	moveside = math.Approach(moveside, -4.25, 0.1)
	movevertical = math.Approach(movevertical, 3, 0.05)

	turn = math.Approach(turn, 0, 0.1)
