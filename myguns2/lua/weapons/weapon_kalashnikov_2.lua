...
function SWEP:GetViewModelPosition(pos, ang)
	...
	turn = math.Approach(turn, 2.25, 10 * FrameTime())
	...
	if self:GetOwner():KeyDown(IN_SPEED) and timerRunning < 1 then
	    ...
	    movevertical = math.Approach(movevertical, 6, 10 * FrameTime())

	    turn = math.Approach(turn, -10, 20 * FrameTime())
	...
	spronetime = 1
	moveside = math.Approach(moveside, -4.25, 15 * FrameTime())
	movevertical = math.Approach(movevertical, 3, 7.5 * FrameTime())

	turn = math.Approach(turn, 0, 25 * FrameTime())
