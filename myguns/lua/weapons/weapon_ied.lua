...
function SWEP:PrimaryAttack()
	...
	timer.Simple(0.65, function()
	     self:GetOwner():StripWeapon("weapon_ied")
	end)
    end
    if (self:Clip1() < 1) then return end
    local prop = ents.Create("prop_physics_override")
    if !IsValid(prop) then return end
    prop:SetKeyValue("ExplodeDamage", "1500")
	...
	timer.Simple(0.015, function()
	    if IsValid(propboom) then
	    	propboom:SetKeyValue("ExplodeDamage", 3500)
    ...
    timer.Simple(0.5, function()
	    ...
	    woosh:Wake()
	    local throwVelocity = self:GetOwner():GetAimVector() * 750
...
function SWEP:Deploy()
    ...
    self:SetNextSecondaryFire(CurTime() + 2 / GetConVar("sv_defaultdeployspeed"):GetFloat())
